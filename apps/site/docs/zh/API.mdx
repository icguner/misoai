...
- 参数：
  ...
  - **新方法：**
    - `prompt: string` - 滚动操作的自然语言描述
    - `options?: Object` - 可选配置：
      - `deepThink?: boolean` - 增强 AI 精度。默认值为 False
      - `cacheable?: boolean` - 当启用 [缓存功能](./caching.mdx) 时，是否允许缓存。默认值为 True

  - **传统方法（已弃用）：**
    - `scrollParam: PlanningActionParamScroll` - 明确的滚动参数
    - `locate?: string` - 元素描述
    - `options?: Object` - 配置选项
...
// 页面滚动
await agent.aiScroll('在产品列表中向下滚动到底部');
await agent.aiScroll('在侧边栏向上滚动200像素');
await agent.aiScroll('在图片轮播图上向左滑动');

// 下拉菜单/选项滚动
await agent.aiScroll('在国家下拉菜单中向下滚动');
await agent.aiScroll('在选项菜单中滚动查找土耳其');
await agent.aiScroll('滚动选择下拉菜单到底部');
await agent.aiScroll('在语言选择菜单中向上滚动');

// 智能搜索滚动
await agent.aiScroll('滚动查找提交按钮');
await agent.aiScroll('滚动直到看到下载链接');
