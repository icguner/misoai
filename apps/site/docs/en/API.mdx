...
AI-powered scroll action using natural language descriptions.

- Type

```typescript
// New simplified approach (recommended)
function aiScroll(prompt: string, options?: Object): Promise<void>;
...
- Parameters:
  ...
  - **New Approach:**
    - `prompt: string` - Natural language description of the scroll action
    - `options?: Object` - Optional configuration:
      - `deepThink?: boolean` - Enhanced AI precision. False by default.
      - `cacheable?: boolean` - Whether cacheable when enabling [caching feature](./caching.mdx). True by default.

  - **Legacy Approach (Deprecated):**
    - `scrollParam: PlanningActionParamScroll` - Explicit scroll parameters
    - `locate?: string` - Element description
...
```typescript
// ✅ New recommended approach - Natural language

// Page scrolling
await agent.aiScroll('scroll down in the product list until bottom');
await agent.aiScroll('scroll up 200 pixels in the sidebar');
await agent.aiScroll('swipe left on the image carousel');

// Dropdown/Menu scrolling
await agent.aiScroll('scroll down in the country dropdown');
await agent.aiScroll('scroll in the options menu to find Turkey');
await agent.aiScroll('scroll the select dropdown until bottom');
await agent.aiScroll('scroll up in the language selection menu');

// Smart search scrolling
await agent.aiScroll('scroll to find the submit button');
await agent.aiScroll('scroll until visible the download link');
...
- **Directions**: "up", "down", "left", "right", "swipe"
- **Targets**: "in the list", "on carousel", "sidebar"
- **Distances**: "200 pixels", "100px", "little bit", "page"
- **Stop conditions**: "until bottom", "until visible", "to find..."
